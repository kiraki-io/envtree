...
program
  ...
  .action(async (options) => {
    ...
    if (comparison.lockFileMethod) {
      console.log('\n📦 Lock File Method:');
      console.log(`  ✅ Found workspace root: ${comparison.lockFileMethod.workspaceRoot}`);
      console.log(`  📄 .env files found: ${comparison.lockFileMethod.envFiles.length}`);
      comparison.lockFileMethod.envFiles.forEach((file: string, index: number) => {
        console.log(`    ${index + 1}. ${file}`);
      });
    } else {
    ...
    if (comparison.workspaceIndicatorMethod) {
      ...
      console.log(`  📄 .env files found: ${comparison.workspaceIndicatorMethod.envFiles.length}`);
      comparison.workspaceIndicatorMethod.envFiles.forEach((file: string, index: number) => {
        console.log(`    ${index + 1}. ${file}`);
      });
    } else {
