 ...
 */
export function findEnvFilesUsingLockFiles(
...
export function findEnvFilesUsingWorkspaceIndicators(
  ...
  while (currentDir !== root) {
    ...
    if (
      isPathAvailable(packageJsonPath) &&
      isWorkspacePackageJson(packageJsonPath)
    ) {
      return {
 ...
 */
export function compareWorkspaceDetectionMethods(
  startDir: string = process.cwd()
): {
  ...
  const lockFileResult = findEnvFilesUsingLockFiles(startDir);
  const workspaceIndicatorResult =
    findEnvFilesUsingWorkspaceIndicators(startDir);

  let recommendation = "";

  if (lockFileResult && workspaceIndicatorResult) {
    if (
      lockFileResult.workspaceRoot === workspaceIndicatorResult.workspaceRoot
    ) {
      recommendation =
